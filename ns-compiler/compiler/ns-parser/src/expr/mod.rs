...
impl Parser {
    fn parse_template_string_literal(&self, raw: &str) -> Option<ns_ast::TemplateStringExpr> {
        ...
        while let Some(rel_start) = raw[cursor..].find("${") {
            ...
            if start > cursor {
                parts.push(ns_ast::TemplateStringPart::Text(
                    raw[cursor..start].to_string(),
                ));
            }
            ...
            parts.push(ns_ast::TemplateStringPart::Expr(Box::new(
                Expr::BindingExpr(BindingExpr(ns_ast::Ident::new(atom(placeholder)))),
            )));
